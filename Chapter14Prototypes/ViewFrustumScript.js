...
// Filename:    ViewFrustumScript.js
// Author:      Don Brutzman
// Identifier:  http://X3dGraphics.com/examples/X3dForWebAuthors/Chapter14-Prototypes/ViewFrustumScript.js
...
function initialize ()
{
    var scriptName = 'ViewFrustumScript';
    ...
    {
        if (trace) Browser.print ('[' + scriptName + '] ' + 'input  ' +
    ...
    if (trace) Browser.print ('[' + scriptName + '] ' + 'ViewFrustum fieldOfView=' + fieldOfView + '\n');
    ...
    if (trace) Browser.print ('[' + scriptName + '] ' +
    ...
    if (trace) Browser.print ('[' + scriptName + '] ' + 'output ' +
...
function set_visible (eventValue)
{
    ...
    else visibilitySwitchSelection = -1;
}
...
function floatArrayToString (inputArray) // overcome irregular .toString() support by X3D browsers
{
    ...
    for (i=0; i < inputArray.length; i++)
    {
        ...
        if (i < inputArray.length - 1) outputString += ' '; // no extra space at end
    }
    return outputString;
}
