...
X3D0 = X3D() \
   ...
   .setHead(head() \
    ...
    .addMeta(meta() \
     .setContent("Kelp Forest Tank at the Monterey Bay Aquarium.") \
     .setName("description") \
    ) \
    .addMeta(meta() \
     .setContent("Mark Boyd") \
     .setName("creator") \
    ) \
    .addMeta(meta() \
     .setContent("11 June 1998") \
     .setName("created") \
    ) \
    .addMeta(meta() \
     .setContent("Jeffrey Weekley, using NIST VRML to X3D Translator, with review and editing in Xeena.") \
     .setName("translator") \
    ) \
    .addMeta(meta() \
    ...
    .addMeta(meta() \
     .setContent("http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/KelpTank.x3d") \
     .setName("identifier") \
    ) \
    .addMeta(meta() \
   ...
   .setScene(Scene() \
    .addChild(Group() \
     .setDEF("FogSwitching") \
     .addChild(Fog() \
      .setDEF("FogOffVisibilityRange0") \
     ) \
     .addChild(Fog() \
      .setDEF("InsideTankFog") \
      .setColor([0.1,0.4,0.4]) \
      .setVisibilityRange(25.0) \
     ) \
     .addChild(ProximitySensor(size = [25.0,10.0,20.0]) \
      .setDEF("CameraInsideTankSensor") \
      .setCenter([0.0,-4.0,-5.0]) \
     ) \
     .addChild(ROUTE() \
      .setFromField("isActive") \
      .setFromNode("CameraInsideTankSensor") \
      .setToField("set_bind") \
      .setToNode("InsideTankFog") \
     ) \
    ) \
    .addChild(Collision() \
     .setEnabled(False) \
     .setProxy(Transform() \
      .addChild(Inline() \
       .setUrl(["KelpTankWaterSurface.x3d","http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/KelpTankWaterSurface.x3d","KelpTankWaterSurface.wrl","http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/KelpTankWaterSurface.wrl"]) \
      ) \
     ) \
     .setProxy(Transform() \
      .addChild(Inline() \
       .setUrl(["KelpTankExternalLights.x3d","http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/KelpTankExternalLights.x3d","KelpTankExternalLights.wrl","http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/KelpTankExternalLights.wrl"]) \
      ) \
     ) \
     .setProxy(Transform() \
      .setRotation([0.0,1.0,0.0,1.5785398]) \
      .setTranslation([-10.35,1.0,-0.5]) \
      .addChild(Inline() \
       .setUrl(["PumpHouse.x3d","http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/PumpHouse.x3d","PumpHouse.wrl","http://X3dGraphics.com/examples/X3dForWebAuthors/KelpForestExhibit/PumpHouse.wrl"]) \
      ) \
     ) \
     .setProxy(Shape() \
      .setAppearance(Appearance() \
       .setDEF("WallAppearance") \
       .setMaterial(Material() \
        .setDiffuseColor([0.251,0.239,0.212]) \
       ) \
      ) \
      .setGeometry(IndexedFaceSet(coordIndex = [0,1,2,3,-1,4,5,6,7,-1,8,9,10,11,-1,16,17,18,19,-1,11,10,3,2,-1,5,4,17,16,-1,20,0,3,21,-1,22,4,7,23,-1,9,24,25,10,-1,17,28,29,18,-1,10,25,21,3,-1,4,22,28,17,-1,30,20,21,31,-1,32,22,23,33,-1,24,34,35,25,-1,28,38,39,29,-1,25,35,31,21,-1,22,32,38,28,-1,40,30,31,41,-1,42,32,33,43,-1,34,44,45,35,-1,38,48,49,39,-1,35,45,41,31,-1,32,42,48,38,-1]) \
       .setCoord(Coordinate() \
        .setDEF("WindowCoordinates") \
        .setPoint([-2.5,-8.5344,0.7,-4.5,-8.5344,2.7,-4.5,-7.21,2.7,-2.5,-7.21,0.7,-2.5,-0.5,0.7,-4.5,-0.5,2.7,-4.5,0.9144,2.7,-2.5,0.9144,0.7,-4.5,-8.5344,3.0,-2.5,-8.5344,1.0,-2.5,-7.21,1.0,-4.5,-7.21,3.0,-4.5,-7.21,2.8,-2.5,-7.21,0.8,-2.5,-0.5,0.8,-4.5,-0.5,2.8,-4.5,-0.5,3.0,-2.5,-0.5,1.0,-2.5,0.9144,1.0,-4.5,0.9144,3.0,0.0,-8.5344,-0.3,0.0,-7.21,-0.3,0.0,-0.5,-0.3,0.0,0.9144,-0.3,0.0,-8.5344,0.0,0.0,-7.21,0.0,0.0,-7.21,-0.2,0.0,-0.5,-0.2,0.0,-0.5,0.0,0.0,0.9144,0.0,2.5,-8.5344,0.7,2.5,-7.21,0.7,2.5,-0.5,0.7,2.5,0.9144,0.7,2.5,-8.5344,1.0,2.5,-7.21,1.0,2.5,-7.21,0.8,2.5,-0.5,0.8,2.5,-0.5,1.0,2.5,0.9144,1.0,4.5,-8.5344,2.7,4.5,-7.21,2.7,4.5,-0.5,2.7,4.5,0.9144,2.7,4.5,-8.5344,3.0,4.5,-7.21,3.0,4.5,-7.21,2.8,4.5,-0.5,2.8,4.5,-0.5,3.0,4.5,0.9144,3.0]) \
       ) \
      ) \
     ) \
     .setProxy(Transform() \
      .addChild(Shape() \
       .setAppearance(Appearance() \
        .setDEF("Glass") \
        .setMaterial(Material() \
         .setDiffuseColor([0.275,0.655,0.62]) \
         .setTransparency(0.63) \
        ) \
       ) \
       .setGeometry(IndexedFaceSet(coordIndex = [3,2,5,4,-1,12,13,14,15,-1,21,3,4,22,-1,13,26,27,14,-1,31,21,22,32,-1,26,36,37,27,-1,41,31,32,42,-1,36,46,47,37,-1]) \
        .setCoord(Coordinate() \
         .setUSE("WindowCoordinates") \
        ) \
       ) \
      ) \
     ) \
     ...
     .setProxy(Transform() \
      .addChild(Shape() \
       ...
       .setGeometry(IndexedFaceSet(coordIndex = [151,150,149,148,147,146,145,144,-1,159,158,157,156,155,154,153,152,-1,128,129,130,131,132,133,134,135,-1,136,137,138,139,140,141,142,143,-1,75,74,73,72,-1,79,78,77,76,-1,42,43,44,45,-1,46,47,48,49,-1,83,82,81,80,-1,87,86,85,84,-1,52,53,54,55,-1,56,57,58,59,-1]) \
        .setCoord(Coordinate() \
         .setUSE("WallCoordinates") \
        ) \
       ) \
     ...
     .setProxy(Shape() \
      ...
      .setGeometry(IndexedFaceSet(coordIndex = [0,1,2,3,-1,7,6,5,4,-1,4,5,1,0,-1,5,6,2,1,-1,6,7,3,2,-1,0,3,7,4,-1]) \
       .setCoord(Coordinate() \
        .setPoint([-0.1,0.91,0.1,0.1,0.91,0.1,0.1,0.91,-0.1,-0.1,0.91,-0.1,-0.1,-7.21,0.1,0.1,-7.21,0.1,0.1,-7.21,-0.1,-0.1,-7.21,-0.1]) \
       ) \
      ) \
     ) \
     .setProxy(Transform() \
      .setDEF("RightBeam") \
      .setRotation([0.0,1.0,0.0,-0.4]) \
      .setTranslation([2.5,0.0,1.0]) \
      .addChild(Shape() \
       .setUSE("Beam") \
      ) \
     ) \
     ...
     .setProxy(Shape() \
      .setAppearance(Appearance() \
       .setUSE("BeamAppearance") \
      ) \
      .setGeometry(IndexedFaceSet(coordIndex = [0,5,6,1,-1,1,6,7,2,-1,2,7,8,3,-1,3,8,9,4,-1,10,0,1,11,-1,11,1,2,12,-1,12,2,3,13,-1,13,3,4,14,-1,14,19,18,13,-1,13,18,17,12,-1,12,17,16,11,-1,11,16,15,10,-1,9,8,18,19,-1,8,7,17,18,-1,7,6,16,17,-1,6,5,15,16,-1]) \
       .setCoord(Coordinate() \
        .setPoint([-4.5,-2.8,3.0,-2.5,-2.8,1.0,0.0,-2.8,0.0,2.5,-2.8,1.0,4.5,-2.8,3.0,-4.5,-3.0,3.0,-2.5,-3.0,1.0,0.0,-3.0,0.0,2.5,-3.0,1.0,4.5,-3.0,3.0,-4.4,-2.8,2.875,-2.5,-2.8,0.75,0.0,-2.8,-0.25,2.5,-2.8,0.75,4.5,-2.8,2.875,-4.5,-3.0,2.875,-2.5,-3.0,0.75,0.0,-3.0,-0.25,2.5,-3.0,0.75,4.5,-3.0,2.875]) \
       ) \
      ) \
     ) \
     .setProxy(Transform() \
      .setTranslation([0.0,-8.5344,0.0]) \
      .addChild(Shape() \
       .setAppearance(Appearance() \
        .setMaterial(Material() \
         .setDiffuseColor([0.4,0.2,0.1]) \
        ) \
       ) \
       .setGeometry(Box(size = [40.0,0.1,40.0]) \
       ) \
      ) \
     ) \
     .setProxy(Transform() \
      .setTranslation([0,0.015,0]) \
      .addChild(Shape() \
       .setAppearance(Appearance() \
        .setMaterial(Material() \
         .setDiffuseColor([0.6,0.545,0.502]) \
        ) \
       ) \
       .setGeometry(IndexedFaceSet(coordIndex = [0,13,1,-1,1,13,2,-1,2,13,3,-1,3,13,14,-1,3,14,4,-1,4,14,15,-1,4,15,5,-1,5,15,16,-1,5,16,6,-1,6,16,17,-1,6,17,7,-1,7,17,18,-1,7,18,8,-1,8,18,19,-1,8,19,9,-1,9,19,20,-1,9,20,10,-1,10,20,11,-1,11,20,13,-1,11,13,12,-1,12,13,0,-1], solid = False) \
        .setCoord(Coordinate() \
         .setPoint([0.0,0.915,-0.3,2.5,0.915,0.7,4.5,0.915,2.7,7.625,0.915,2.7,9.95,0.915,0.375,9.95,0.915,-4.375,6.375,0.915,-7.7,-5.5,0.915,-7.7,-10.2,0.915,-2.875,-10.2,0.915,0.25,-7.625,0.915,2.7,-4.5,0.915,2.7,-2.5,0.915,0.7,0.0,1.01,10.0,15.0,1.01,10.0,15.0,1.01,0.0,15.0,1.01,-15.0,0.0,1.01,-15.0,-15.0,1.01,-15.0,-15.0,1.01,0.0,-15.0,1.01,10.0]) \
        ) \
       ) \
      ) \
     ) \
     .setProxy(Transform() \
      .setTranslation([-5.125,0.5,-6.875]) \
      .addChild(Shape() \
       .setAppearance(Appearance() \
        .setMaterial(Material() \
         .setDiffuseColor([0.5451,0.5333,0.4745]) \
         .setShininess(0.0) \
        ) \
       ) \
       .setGeometry(Cylinder(height = 3.0) \
       ) \
      ) \
     ) \
    ) \
...
X3D0.toFileX3D("/x3d-code/www.web3d.org/x3d/content/examples/X3dForWebAuthors/KelpForestExhibit//KelpTank_RoundTrip.x3d")
